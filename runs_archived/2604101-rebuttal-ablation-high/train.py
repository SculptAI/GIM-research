...
num_mid = configs.DATASET_LEN // 2
num_low = configs.DATASET_LEN - num_mid

mid_dataset = mid_dataset.select(rng.sample(range(len(mid_dataset)), k=num_mid))
low_dataset = low_dataset.select(rng.sample(range(len(low_dataset)), k=num_low))

dataset = concatenate_datasets([mid_dataset, low_dataset]).shuffle(seed=configs.RANDOM_SEED)

assert len(dataset) == configs.DATASET_LEN, f"{len(dataset)=}, {configs.DATASET_LEN=}"
logging.info(f"Number of training samples: mid={len(mid_dataset)}, low={len(low_dataset)}")
