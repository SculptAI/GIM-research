...
def long_example(example):
    return len(example["gim_query"]) + len(example["gim_response"]) > 2000


logging.info("Loading and preparing dataset...")
rng = random.Random(configs.RANDOM_SEED)
high_dataset = _concat_subsets(high_subsets).shuffle(seed=configs.RANDOM_SEED).filter(long_example)
mid_dataset = _concat_subsets(mid_subsets).shuffle(seed=configs.RANDOM_SEED).filter(long_example)
low_dataset = _concat_subsets(low_subsets).shuffle(seed=configs.RANDOM_SEED).filter(long_example)

num_high = len(high_dataset)
num_mid = (configs.DATASET_LEN - num_high) // 2
